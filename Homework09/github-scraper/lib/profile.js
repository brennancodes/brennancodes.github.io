...
module.exports = function profile ($, url, callback) {
  ...
  data.bio = $('.user-profile-bio').first().text().trim();
  ...
  data.location = $('.octicon-location').parent().first().text().trim(); // Location
  data.name = $('.vcard-fullname').text().trim();            // Full Name
  // data.email    = $('.email').text();                     // email address
  data.website  = $('.octicon-link').next().first().text();     // Website
